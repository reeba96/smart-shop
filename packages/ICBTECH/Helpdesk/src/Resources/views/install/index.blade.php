...
<div class="container">
    <h1 style="text-align: center">{{ trans('helpdesk::install.initial-setup') }}</h1>
  <form class="form-signin" action="{{url('/tickets-install') }}" method="post" style="max-width: 500px">
          <input type="hidden" name="_token" value="{{ csrf_token() }}" />
        <h3 class="form-signin-heading">{{ trans('helpdesk::install.master-template-file') }}</h3>
        ...
        <div class="form-group" id="other-path-group" style="display: none">
            <label for="other_path">{{ trans('helpdesk::install.master-template-other-path') }}</label>
            <input type="text" id="other_path" name="other_path" class="form-control" />
            <span id="helpBlock" class="help-block">
                {{ trans('helpdesk::install.master-template-other-path-ex') }}
            </span>
        </div>

        <h3 class="form-signin-heading">{{ trans('helpdesk::install.admin-select') }}</h3>
        ...
        <span id="helpBlock" class="help-block">
            {{ trans('helpdesk::install.admin-select-help-block') }}
        </span>
        <br>

        <div class="well small" style="border: 1px solid #ccc">
            @if(!empty($inactive_migrations))
                <b>{{ trans('helpdesk::install.migrations-to-be-installed') }}</b>
                <ul>
            ...
            @else
                <b>{{ trans('helpdesk::install.all-tables-migrated') }}</b>
            @endif
        </div>
        <br>
        <button class="btn btn-lg btn-primary btn-block" type="submit">
            {{ trans('helpdesk::install.proceed') }}
        </button>
    </form>
